...
def process_image_with_ai_background(
    ...
    try:
        ...
        # A variável chama-se prompt_result
        ...
        # --- INÍCIO DA CORREÇÃO ---
        # Só executamos o Asset Crew se a intenção for adicionar itens e a lista não estiver vazia
        if prompt_result.intent == "add" and prompt_result.items_to_add:
            ...
            asset_crew_result = asset_crew.kickoff(
                inputs={
                    "optimized_prompt": optimized_prompt,
                    "generated_image_url": output_url,
                    # Usamos prompt_result.items_to_add em vez de architect_result
                    "items_to_create": ", ".join(prompt_result.items_to_add)
                }
            )
            ...
            print(f"[Project {project_id}] Intenção é '{prompt_result.intent}'. A saltar a criação de assets.")
        # --- FIM DA CORREÇÃO ---
        ...
        db.commit()
        print(f"🎉 [CrewAI SUCCESS] Processamento concluído com sucesso: {output_url}")
