...
class UserModel(Base):
    ...
    last_generation_date: Mapped[date] = mapped_column(
    ...
    saved_items: Mapped[List["UserSavedItemModel"]] = relationship(
        back_populates="user", cascade="all, delete-orphan"
    )
...
class UserIdentityModel(Base):
    ...
    )  # ID que vem do Google (vazio se for local)
    password_hash: Mapped[str] = mapped_column(
        String, nullable=True
    )  # Hash da password (vazio se for google)
